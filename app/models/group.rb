class Group < ActiveRecord::Base
  ...
  has_many :students, -> {
    where(memberships: { deleted_at: nil }).order(:surname, :name, :patronymic)
  }, through: :memberships, source: :person, source_type: 'Person', class_name: 'Student'
  ...
  def students_at(date)
    memberships.where('created_at < :date AND deleted_at IS NULL OR deleted_at > :date', date: date)
      .map(&:person)
      .uniq.sort_by{ |p| p[:surname] }
  end
